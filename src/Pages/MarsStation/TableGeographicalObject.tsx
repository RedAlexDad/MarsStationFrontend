...
export default function TableGeographicalObject({sortedLocations, photoUrlsMap, geographicalObjectMap, selectedMarsStation, deleteLocationAndMarsStation, putLocationAndMarsStation}: {
    sortedLocations: any[];
                ...
                <TableHead>
                    <TableRow>
                        ...
                        {STATUS_TASKS.find(status => status.id === selectedMarsStation?.status_task)?.name === "Черновик" &&
                            <>
                                <TableCell
                                    style={{fontSize: '18px', fontWeight: 'bold', color: 'white'}}>Перемещение</TableCell>
                                <TableCell
                ...
                <TableBody>
                    {sortedLocations.map((location) => (
                        <TableRow key={location.sequence_number}>
                            ...
                            <TableCell
                                style={{fontSize: '18px', color: 'white'}}>{geographicalObjectMap[location.id_geographical_object]?.feature}</TableCell>
                            <TableCell
                                style={{fontSize: '18px', color: 'white'}}>{geographicalObjectMap[location.id_geographical_object]?.type}</TableCell>
                                    ...
                                    <TableCell>
                                        {sortedLocations.length > 1 && (
                                            <ButtonGroup orientation="vertical">
                                                ...
                                                {location.sequence_number !== sortedLocations.length && (
                                                    <Button
                                                        variant="outlined"
                                                        style={{color: 'white', borderColor: 'white'}}
